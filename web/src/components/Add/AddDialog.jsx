...
export default function AddDialog({
  ...
  const getAllHashes = () => torrents?.map(({ hash }) => hash) ?? []

  const doesHashMatchCurrentSource = hash => {
    if (!currentSourceHash) return false

    return hash?.toLowerCase().startsWith(currentSourceHash.toLowerCase())
  }
  ...
  useEffect(() => {
    ...
    setIsHashAlreadyExists(allHashes.some(doesHashMatchCurrentSource))
  ...
  useEffect(() => {
    ...
    const allHashes = getAllHashes()
    allHashes.some(doesHashMatchCurrentSource) && handleClose()

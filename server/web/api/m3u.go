...
func allPlayList(c *gin.Context) {
	user := apiutils.UserID(c)
	torrs := torr.ListTorrent(user)
	...
	for _, tr := range torrs {
		...
		list += host + "/stream/" + url.PathEscape(tr.Title) + ".m3u?link=" + apiutils.JoinHashUser(tr.Hash().HexString(), user) + "&m3u&fn=file.m3u\n"
...
func playList(c *gin.Context) {
	...
	var ok bool
	hash, user, ok = apiutils.ResolveHashUser(c, hash, user)
	if !ok {
	...
	host := utils.GetScheme(c) + "://" + c.Request.Host
	list := getM3uList(tor.Status(), host, user, fromlast)
	list = "#EXTM3U\n" + list
...
func getM3uList(tor *state.TorrentStatus, host string, user string, fromLast bool) string {
	m3u := ""
	from := 0
	if fromLast {
			...
			if utils.GetMimeType(f.Path) != "*/*" {
				...
				if fileNamesakes != nil {
					...
					for _, namesake := range fileNamesakes { // include play-links to external media, with # splitter
						...
						m3u += host + "/stream/" + url.PathEscape(sname) + "?link=" + apiutils.JoinHashUser(tor.Hash, user) + "&index=" + fmt.Sprint(namesake.Id) + "&play#"
					}
					m3u += "\n"
				}
				name := filepath.Base(f.Path)
				m3u += host + "/stream/" + url.PathEscape(name) + "?link=" + apiutils.JoinHashUser(tor.Hash, user) + "&index=" + fmt.Sprint(f.Id) + "&play\n"
			}
		}
	}
	return m3u
...
func searchLastPlayed(tor *state.TorrentStatus, user string) int {
	viewed := sets.ListViewed(tor.Hash, user)
	if len(viewed) == 0 {
		return -1
	}

...
func stream(c *gin.Context) {
	...
	if !strings.Contains(link, "://") && !strings.HasPrefix(link, "magnet:?") {
		var ok bool
		link, user, ok = utils.ResolveHashUser(c, link, user)
		if !ok {
	...
	tor := torr.GetTorrent(user, spec.InfoHash.HexString())
	if tor != nil {
		title = tor.Title
		poster = tor.Poster
		data = tor.Data
	...
	if tor == nil || tor.Stat == state.TorrentInDB {
		tor, err = torr.AddTorrent(user, spec, title, poster, data, category)
		if err != nil {
			c.AbortWithError(http.StatusInternalServerError, err)
			return
		}
	...
	if stat {
		status := tor.Status()
		status.Hash = utils.JoinHashUser(status.Hash, user)
		c.JSON(200, status)
		return
	} else
	// return m3u if query
	if m3u {
		...
		m3ulist := "#EXTM3U\n" + getM3uList(tor.Status(), utils2.GetScheme(c)+"://"+c.Request.Host, user, fromlast)

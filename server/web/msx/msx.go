...
func hashUser(data, user string) (string, string) {
	last := strings.LastIndexByte(data, ':')
	if last != -1 {
		if prev := strings.LastIndexByte(data[:last], ':'); prev != -1 {
			data = data[prev+1:]
		} else {
			data = data[last+1:]
		}
	...
	return apiutils.SplitHashUser(data, user)
}

func trn(h, user string) (st, sc string) {
	if h := torr.GetTorrent(user, h); h != nil {
...
func SetupRoute(r gin.IRouter) {
	...
	authorized.GET("/msx/trn", func(c *gin.Context) {
		...
		if h := c.Query("hash"); h != "" {
			var u string
			if h, u = hashUser(h, auth.GetUserID(c)); h == "" {
				c.JSON(http.StatusOK, false)
				return
			}
			for _, t := range settings.ListTorrent(u) {
	...
	authorized.POST("/msx/trn", func(c *gin.Context) {
		...
		if j := struct{ Data string }{Data: c.Query("hash")}; j.Data != "" {
			h, u := hashUser(j.Data, auth.GetUserID(c))
			st, sc := trn(h, u)
			if sc != "" {
				sc = "{col:" + sc + "}"
			}

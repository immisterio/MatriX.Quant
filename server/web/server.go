...
func Start() {
	...
	if err := connectBTServers(); err != nil {
...
func Stop() {
	torr.DisconnectAllServers()
	waitChan <- nil
}

func connectBTServers() error {
	users := make([]string, 0)

	if settings.HttpAuth {
		for user := range auth.Accounts() {
	...
	for _, user := range users {
		if _, err := torr.ConnectServer(user); err != nil {
			return err
		}
	}

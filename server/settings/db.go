...
// normalizeUserID returns a stable bucket name for a user.
// Empty user IDs are mapped to "base" and slashes are replaced to avoid nested buckets.
...
// joinUserXPath builds an XPath string that includes the user bucket.
func joinUserXPath(xpath, user string) string {
	user = normalizeUserID(user)
	if xpath == "" {
		return user
	}
	return xpath + "/" + user
